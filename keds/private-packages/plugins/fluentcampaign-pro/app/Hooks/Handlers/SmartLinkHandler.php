...
class SmartLinkHandler
{
    ...
    {
        add_action('fluentcrm_smartlink_clicked', [$this, 'handleClick'], 10, 1);
        add_action('fluentcrm_smartlink_clicked_direct', [$this, 'handleClick'], 10, 1);

        add_filter('fluent_crm/smartlink_by_short_url', [$this, 'getSmartLinkByShortUrl'], 10, 2);

        add_filter('fluent_crm/has_smartlink', function ($status) {
            return ProHelper::hasSmartLink();
        }, 999);
    ...
    public function handleClick($slug, $contact = null)
    {
        ...
        if (strpos($targetUrl, '{{')) {
            ...
            $targetUrl = preg_replace_callback('/([^&=]*)=([^&]*)/', function ($matches) {
    ...
    public function getSmartLinkByShortUrl($result, $shortUrl)
    {
        if (!ProHelper::hasSmartLink()) {
            return $result;
        }

        $urlParts = wp_parse_url($shortUrl);

        $slug = '';
        // check if we have route query param
        if (isset($urlParts['query'])) {
            parse_str($urlParts['query'], $query);
            if ($query && isset($query['route']) && $query['route'] === 'smart_url' && isset($query['slug'])) {
                $slug = sanitize_text_field($query['slug']);
            } else {
                return $result;
            }
        }
        ...
        return SmartLink::where('short', $slug)->first();
    }
