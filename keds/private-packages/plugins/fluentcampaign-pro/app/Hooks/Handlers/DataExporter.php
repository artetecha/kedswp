...
class DataExporter
{
    ...
    /**
     * @deprecated Use getContactColumnMaps() instead
     */
    private function contactColumnMaps()
    {
        return self::getContactColumnMaps();
    }

    public static function getContactColumnMaps()
    {
        return [
    ...
    public function importEmailTemplate()
    {
        ...
        $hasVisualBuilderDesign = Arr::get($data, 'design_template') == 'visual_builder' && !empty($data['_visual_builder_design']);

        if (empty($data['post_title']) || (empty($data['post_content']) && !$hasVisualBuilderDesign) || Arr::get($data, 'is_fc_template') != 'yes') {
            wp_send_json([
                'message'  => __('The provided JSON file is not valid.', 'fluentcampaign-pro'),
                'requires' => [
        ...
        if ($editType = Arr::get($data, 'edit_type', Arr::get($data, '_edit_type'))) {
    ...
    /**
     * @deprecated Use sanitizeForCSVValue() instead
     */
    private function sanitizeForCSV($content)
    {
        return self::sanitizeForCSVValue($content);
    }

    public static function sanitizeForCSVValue($content)
    {
    ...
    /**
     * Build ordered header labels for export columns.
     *
     * @param array $columns        Selected column keys
     * @param array $customFields   Custom field slugs
     * @param array $commerceColumns Commerce column keys
     * @return array{labels: string[], header_keys: string[], custom_headers: string[]}
     *               - labels:         ordered display labels for every column (standard + custom + commerce)
     *               - header_keys:    standard column keys (matches $columns intersection of contactColumnMaps)
     *               - custom_headers: custom field slugs that resolved to existing fields
     */
    public static function buildExportHeaders($columns, $customFields = [], $commerceColumns = [])
    {
        $maps = self::getContactColumnMaps();
        ...
        if ($commerceColumns) {
            foreach ($commerceColumns as $column) {
                $labels[] = ucwords(implode(' ', explode('_', $column)));
            }
        }
    ...
    /**
     * Flatten a subscriber into an indexed array of export values.
     *
     * @param \FluentCrm\App\Models\Subscriber $subscriber
     * @param array $headerKeys      Standard column keys (from buildExportHeaders)
     * @param array $customHeaders   Custom field slugs (from buildExportHeaders)
     * @param array $commerceColumns Commerce column keys
     * @return array Indexed array of sanitized string values
     */
    public static function flattenSubscriberRow($subscriber, $headerKeys, $customHeaders = [], $commerceColumns = [])
    {
        $row = [];
        ...
        foreach ($headerKeys as $headerKey) {
            if (in_array($headerKey, ['lists', 'tags', 'companies'])) {
                ...
                $row[] = ($subscriber->company && isset($subscriber->company->name))
                    ? $subscriber->company->name
                    : '';
            } else {
                $value = $subscriber->{$headerKey};
        ...
        if ($commerceColumns) {
            foreach ($commerceColumns as $column) {
                if ($subscriber->commerce_by_provider && isset($subscriber->commerce_by_provider->{$column})) {
                    $value = $subscriber->commerce_by_provider->{$column};
                    $row[] = ($value === null) ? '' : (string) $value;
                } else {
                    $row[] = '';
                }
        ...
        // Sanitize every cell to prevent CSV formula injection (=, +, -, @, \t, \r).
        // Defense in depth: client-side CSV writer also sanitizes, but server-side
        // protection ensures any other API consumer also gets safe output.
        return array_map([self::class, 'sanitizeForCSVValue'], $row);
    }
    ...
    public function exportSMSCampaign()
    {
        $this->verifyRequest('fcrm_manage_sms');
        $campaignId = (int)$_REQUEST['campaign_id'];

        if (!$campaignId) {
            die(__('Please provide Valid SMS Campaign ID', 'fluentcampaign-pro'));
        }
        ...
        $campaignData = apply_filters('fluent_crm/sms_campaign_export_data', $campaignData, $campaign);
        $fileName = 'sms-campaign-' . sanitize_file_name(sanitize_title($campaign->title)) . '-' . $campaign->id . '.json';
        if (!headers_sent()) {
            header('Content-Disposition: attachment; filename=' . $fileName);
            header('Content-Type: application/json');
        }
    ...
    public function importSMSCampaign()
    {
        $this->verifyRequest('fcrm_manage_sms');
        ...
        $campaignData['title'] = empty($campaignData['title'])
            ? 'Imported SMS Campaigns @ ' . current_time('mysql')
            : '[Imported] ' . sanitize_text_field($campaignData['title']) . ' - ' . current_time('mysql');
        ...
        $formattedData['message_content'] = wp_kses($campaignData['message_content'], [
            'a' => [
                'href' => [],
                'title' => [],
                'target' => []
        ...
        $campaign = SMSCampaign::create($formattedData);

        if (is_wp_error($campaign)) {
            wp_send_json([
                'message'  => $campaign->get_error_message(),
                'requires' => [
                    __('Could not create the sms campaign', 'fluentcampaign-pro')
                ]
            ], 423);
        }
        ...
        if (isset($campaignData['labels']) && !empty($campaignData['labels'])) {
            ...
            if (!empty($labelIds)) {
                $campaign->labelsTerm()->syncWithoutDetaching($labelIds);
            }
        }

        wp_send_json([
            'message' => __('SMS Campaigns has been successfully imported', 'fluentcampaign-pro'),
            'campaign_id' => $campaign['id']
        ]);
    }
