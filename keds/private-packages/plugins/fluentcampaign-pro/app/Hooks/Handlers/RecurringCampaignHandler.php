...
class RecurringCampaignHandler
{
    public function maybePushNewEmailDraft()
    {
        ...
        $compareTime = gmdate('Y-m-d H:i:s', current_time('timestamp') + 2100);
        $campaignBatchLimit = (int)apply_filters('fluent_crm/recurring_campaign_batch_limit', 10);
        if ($campaignBatchLimit < 1) {
            $campaignBatchLimit = 1;
        }
        ...
        foreach ($campaigns as $campaign) {
            ...
            do_action('fluent_crm/recurring_mail_created', $recurringMail, $campaign);
    ...
    public function willProcessCampaign($campaign)
    {
        ...
        if($frequency == 'daily') {
            ...
            $cutoff = 6 * 86400; // 6 days
        } elseif ($frequency == 'monthly') {
            $cutoff = 27 * 86400; // 27 days
        }

        // check if there has any created campaign email in the last 18 hours
        $exist = RecurringMail::where('parent_id', $campaign->id)
            ->where('created_at', '>=', gmdate('Y-m-d H:i:s', current_time('timestamp') - $cutoff))
            ->first();
        ...
        foreach ($conditions as $conditionBlock) {
            ...
            foreach ($conditionBlock as $condition) {
                $isPassed = RecurringCampaignRunner::assessCondition($condition);
                if (!$isPassed) {
                    break;
                }
            }
