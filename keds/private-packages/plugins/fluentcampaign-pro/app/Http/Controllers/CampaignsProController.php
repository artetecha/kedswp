...
class CampaignsProController extends Controller
{
    ...
    public function resendEmails(Request $request, $campaignId)
    {
        ...
        foreach ($emails as $email) {
            if (!$email->subscriber) {
                if(count($emails) == 1) {
                    return $this->sendError([
                        'message' => 'Sorry, email cannot be sent as subscriber could not be found'
                    ]);
                }
                continue;
            }
            ...
            if (!$email->email_body) {
                if(count($emails) == 1) {
                    return $this->sendError([
                        'message' => 'Sorry, email cannot be sent as email body is empty'
                    ]);
                }
                continue;
            }
    ...
    public function resendUnopenedEmails(Request $request, $campaignId)
    {
        $campaign = Campaign::withoutGlobalScopes()->findOrFail($campaignId);
        ...
        $emails = CampaignEmail::where('campaign_id', $campaignId)
            ->with('subscriber')
            ->whereIn('status', ['sent', 'failed'])
            ->where('is_open', 0) // unopened emails
            ->get();


        if ($emails->isEmpty()) {
            return $this->sendError([
                'message' => __('Sorry! No unopened emails found', 'fluentcampaign-pro')
            ]);
        }
        ...
        return [
            'message' => __('Unopened Emails has been resent', 'fluentcampaign-pro')
        ];
    }
