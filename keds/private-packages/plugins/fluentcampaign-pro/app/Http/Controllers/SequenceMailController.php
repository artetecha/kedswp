...
class SequenceMailController extends Controller
{
        ...
        if ($emailId) {
            ...
            if (!$email) {
                return $this->sendError([
                    'message' => __('Sequence email could not be found', 'fluentcampaign-pro')
                ]);
            }
    ...
    {
        $data = Helper::parseArrayOrJson($request->get('email'));
    ...
    {
        $emailId = (int)$request->get('email_id');
        if (!$emailId) {
            return $this->sendError([
                'message' => __('Please provide email_id for duplication', 'fluentcampaign-pro')
            ]);
        }
    ...
    /*
    * Not Derectly used by api/routes 
    * Used by $this->routeFallBackSequenceEmailCreateUpdate() method
    */
    ...
    public function updateDelay(Request $request, $sequenceId, $emailId)
    {
        $email = SequenceMail::where('parent_id', $sequenceId)->findOrFail($emailId);

        $delay = sanitize_text_field($request->get('delay', '0'));
        $delayUnit = sanitize_text_field($request->get('delay_unit', 'days'));

        $allowedUnits = ['minutes', 'hours', 'days', 'weeks', 'Months'];
        if (!in_array($delayUnit, $allowedUnits)) {
            $delayUnit = 'days';
        }

        $settings = $email->settings;
        $settings['timings']['delay'] = $delay;
        $settings['timings']['delay_unit'] = $delayUnit;
        $email->settings = $settings;
        $email->save();

        return $this->sendSuccess([
            'message' => __('Wait time has been updated', 'fluentcampaign-pro'),
            'email'   => $email
        ]);
    }
