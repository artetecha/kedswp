...
class DynamicSegmentController extends Controller
{
    ...
    public function getStats()
    {
        ...
        foreach ($segments as $segment) {
            $segmentData = apply_filters('fluentcrm_dynamic_segment_' . $segment['slug'], null, $segment['id'], [
                'subscribers' => false,
                'paginate'    => false,
                'model'       => true,
            ]);
            ...
            $model = $segmentData['model'];
            $stats[$segment['slug'].'_'.$segment['id']] = $model->count();
        }
    ...
    public function getSegment(Request $request, $slug, $id = 0)
    {
        ...
        // Dynamic segments return Subscriber-shaped queries (see ->searchBy /
        // tags / lists / commerce_by_provider below), so the allowlist is the
        // fc_subscribers column set. Required because the framework rewrite
        // made orderBy() throw on names that don't match ^[a-zA-Z0-9_\.]+$.
        $allowedSortBy = [
        ...
        $sortBy = sanitize_key((string) $this->request->get('sort_by'));
        $sortType = strtoupper((string) $this->request->get('sort_type'));
        if ($sortBy !== '' && in_array($sortBy, $allowedSortBy, true)) {
            $sortType = in_array($sortType, ['ASC', 'DESC'], true) ? $sortType : 'DESC';
            $model->orderBy($sortBy, $sortType);
        }

        $subscribers = $model->with(['tags', 'lists']);

        if (Helper::isCompanyEnabled()) {
            $subscribers = $subscribers->with('companies');
        }
    ...
    public function duplicate(Request $request, $segmentId)
    {
        $segment = Meta::find($segmentId)->toArray();
        unset($segment['id']);
        $title = __('[Duplicate] ', 'fluentcampaign-pro') . $segment['value']['title'];

        $segment['created_at'] = fluentCrmTimestamp();
        $segment['value']['title'] = $title;
        $inserted = Meta::create($segment);

        return $this->sendSuccess([
            'message'     => __('Segment successfully duplicated', 'fluentcampaign-pro'),
