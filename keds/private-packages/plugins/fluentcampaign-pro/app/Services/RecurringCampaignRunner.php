...
class RecurringCampaignRunner
{
    ...
    public static function getNextScheduledAt($schedulingSettings, $cutSeconds = 1800)
    {
        ...
        switch ($type) {
            ...
            case 'monthly':
                $selectedDay = max(1, absint(Arr::get($schedulingSettings, 'day', '1')));
                $currentDay = (int) gmdate('j', $currentTimeStamp);
                $daysInCurrentMonth = (int) gmdate('t', $currentTimeStamp);
                $clampedCurrentDay = min($selectedDay, $daysInCurrentMonth);

                if ($clampedCurrentDay == $currentDay) {
                    // it's today (or clamped to today for short months)
                    $nextDateTime = gmdate('Y-m-d', $currentTimeStamp) . ' ' . $time;
                    if (strtotime($nextDateTime) - $currentTimeStamp > $cutSeconds) {
                        return $nextDateTime; // it's after the cut seconds
                    }
                } else if ($currentDay < $clampedCurrentDay) {
                    $paddedDay = str_pad($clampedCurrentDay, 2, '0', STR_PAD_LEFT);
                    return gmdate('Y-m-', $currentTimeStamp) . $paddedDay . ' ' . $time;
                }

                $nextMonth = strtotime("first day of next month", $currentTimeStamp);
                $daysInNextMonth = (int) gmdate('t', $nextMonth);
                $clampedNextDay = min($selectedDay, $daysInNextMonth);
                $advanceDay = $clampedNextDay - 1;
    ...
    public static function assessCondition($condition)
    {
