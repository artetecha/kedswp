...
class SureCartInit
{
    ...
    public function getPurchaseHistory($data, $subscriber)
    {
        ...
        foreach ($orders->data  as $order) {
            $status = '<span class="fcrm_badge fcrm_badge_'.esc_attr($order->status).'">'. esc_html(\FluentCrm\App\Services\Helper::getStatusText($order->status)) .'</span>';
            $formattedOrders[] = [
        ...
        $returnData = [
            ...
            'columns_config' => [
                'Order' => [ 'label' => __('Order', 'fluentcampaign-pro'), 'sortable' => false, 'key' => 'number'],
                'type' => [ 'label' => __('Type', 'fluentcampaign-pro'), 'sortable' => false, 'key' => 'order_type'],
                'status' => [ 'label' => __('Status', 'fluentcampaign-pro'), 'sortable' => false, 'key' => 'status'],
                'date' => [ 'label' => __('Date', 'fluentcampaign-pro'), 'sortable' => false, 'key' => 'created_at'],
                'total' => [ 'label' => __('Total', 'fluentcampaign-pro'), 'sortable' => false, 'key' => 'total_amount'],
            ]
        ];
    ...
    {
        $providers['surecart'] = [
            'title'       => __('SureCart', 'fluentcampaign-pro'),
            'sub_title'   => __('With SureCart integration with FluentCRM, you can trigger automations on successful payments and refunds, track purchase history, segment customers by products purchased, and view customer profiles with order details.', 'fluentcampaign-pro'),
            'type'        => 'commerce',
            'is_enabled' => true,
            'settings'    => null
        ];
