...
class AdvancedReport extends BaseAdvancedReport
{
    ...
    {
        $enabled = Commerce::isEnabled('learndash');
        ...
        $supports = [
            'product_growth'   => [
                ...
                'sub_types' => [
                    ...
                    'memberships' => [
                        'label' => __('Membership Groups', 'fluentcampaign-pro')
                    ]
                ]
            ],
            'customers_growth' => [
        ...
        $overview = [
            'enabled'       => $enabled,
            'title'         => __('LearnDash - Advanced Reports', 'fluentcampaign-pro'),
            'supports'      => $supports,
            'has_top_products_filter' => true
        ];

        if (!$enabled) {
            $overview['enable_instruction'] = __('Please enable data sync first from FluentCRM', 'fluentcampaign-pro').' <b><a href="'.admin_url('admin.php?page=fluentcrm-admin#/settings/integrations?selected_integration=learndash').'">'.__('Settings -> Integrations Settings -> LearnDash', 'fluentcampaign-pro').'</a></b> '.__('to view in details LearnDash reports', 'fluentcampaign-pro');
        } else {
            $overview['widgets'] = [
                'total_students'         => [
