...
class AdvancedReport extends BaseAdvancedReport
{
    ...
    public function getReports($filters = [])
    {
        ...
        $overview = [
            'enabled'       => $enabled,
            'title'         => __('TutorLms - Advanced Reports', 'fluentcampaign-pro'),
            'supports'      => $supports,
            'has_top_products_filter' => true
        ];

        if (!$enabled) {
            $overview['enable_instruction'] = __('Please enable data sync first from FluentCRM', 'fluentcampaign-pro').' <b><a href="'.admin_url('admin.php?page=fluentcrm-admin#/settings/integrations?selected_integration=tutorlms').'">'.__('Settings -> Integrations Settings -> TutorLms', 'fluentcampaign-pro').'</a></b> '.__('to view in details TutorLms reports', 'fluentcampaign-pro');
        } else {
            $overview['widgets'] = [
                'total_students'         => [
