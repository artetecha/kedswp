...
class DeepIntegration
{
    ...
    public function init()
    {
        ...
        add_filter('fluent_crm/advanced_report_providers', function ($providers) {
            $providers['tutorlms'] = [
                ...
                'icon' => fluentCrmMix('images/tutorlms.jpg')
            ];
            return $providers;
        }, 10, 1);
        ...
        add_filter('fluentcrm_advanced_filter_suggestions', function ($suggestions) {
            if (!Commerce::isEnabled($this->integrationkey)) {
                $suggestions[] = [
                    ...
                    'provider' => 'tutorlms',
                    'btn_url'  => admin_url('admin.php?page=fluentcrm-admin#/settings/integrations?selected_integration=tutorlms')
                ];
            }
    ...
    private function getCompletedCourses($userId)
    {
        ...
        return $completedCourses->pluck('comment_post_ID')->toArray();
    }
