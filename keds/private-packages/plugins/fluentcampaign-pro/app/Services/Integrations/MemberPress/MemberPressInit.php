...
class MemberPressInit
{
    ...
    public function pushSubscriberInfoWidget($widgets, $subscriber)
    {
        ...
        if ($subscriptionList->isEmpty() && $transactionList->isEmpty()) {
            return $widgets;
        }
    ...
    {
        $providers['memberpress'] = [
            'title'       => __('MemberPress', 'fluentcampaign-pro'),
            'sub_title'   => __('With MemberPress integration with FluentCRM, you can automate actions based on membership enrollment and subscription expiration, track member subscriptions including recurring and one-time purchases, and view detailed membership history with status updates.', 'fluentcampaign-pro'),
            'type'        => 'membership',
            'is_enabled'  => true,
            'settings'    => null
        ];
