...
class Helper
{
    ...
    /**
     * Check whether a subscriber purchased any selected product in EDD 3 orders.
     */
    public static function isProductPurchased($productIds, $subscriber)
    {
        $productIds = array_filter(array_map('absint', (array) $productIds));

        if (!$productIds || !self::isEdd3()) {
            return false;
        }
        ...
        /*
         * EDD 3 stores purchases in edd_orders and order lines in
         * edd_order_items, so avoid the legacy payment-query layer.
         */
        $ordersQuery = fluentCrmDb()->table('edd_orders')
            ->join('edd_order_items', 'edd_orders.id', '=', 'edd_order_items.order_id')
            ->where('edd_orders.type', 'sale')
            ->whereIn('edd_orders.status', ['processing', 'complete', 'completed', 'partially_refunded'])
            ->whereIn('edd_order_items.product_id', $productIds);

        if ($userId) {
            $ordersQuery = $ordersQuery->where('edd_orders.user_id', $userId);
        }
        ...
        if (!$userId) {
            ...
            $ordersQuery = $ordersQuery->where('edd_orders.customer_id', $customer->id);
        }
    ...
    /**
     * Proxy the shared FluentCRM EDD support check for Pro EDD modules.
     *
     * @return bool
     */
    public static function isEdd3()
    {
        return method_exists(CrmHelper::class, 'isEdd3') && CrmHelper::isEdd3();
    }
}
