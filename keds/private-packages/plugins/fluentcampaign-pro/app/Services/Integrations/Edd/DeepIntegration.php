...
class DeepIntegration
{
    public function init()
    {
        ...
        add_filter('fluent_crm/advanced_report_providers', function ($providers) {
            ...
            if (apply_filters('fluent_crm/user_can_view_edd_report', current_user_can('view_shop_sensitive_data'))) {
                $providers['edd'] = [
                    'title' => 'EDD',
                    'icon' => fluentCrmMix('images/edd.svg')
                ];
            }
        ...
        add_filter('fluentcrm_advanced_filter_suggestions', function ($suggestions) {
            if (!Commerce::isEnabled('edd')) {
                $suggestions[] = [
                    ...
                    'provider' => 'edd',
                    'btn_url'  => admin_url('admin.php?page=fluentcrm-admin#/settings/integrations?selected_integration=edd')
                ];
            }
    ...
    /**
     * Resync EDD customer data after relevant EDD 3 order status changes.
     *
     * @param int $paymentId
    ...
    public function handlePaymentStatusChanged($paymentId, $newStatus, $oldStatus)
    {
        ...
        $resyncTypes = ['processing', 'complete', 'completed', 'refunded', 'partially_refunded'];
    ...
    public function pushSubscriberInfoWidget($widgets, $subscriber)
    {
        ...
        $commerce = $this->getCommerceRecord($subscriber->id);
        if (!$commerce) {
            return $widgets;
        }
        ...
        $widgets[] = [
            'title'   => '',
            'content' => $this->buildCommerceWidgetHtml($commerce)
        ];

        return $widgets;
    }

    private function getCommerceRecord($subscriberId)
    {
        return ContactRelationModel::provider('edd')
    ...
    private function buildCommerceWidgetHtml($commerce)
    {
        ...
        if(Commerce::getCommerceProvider() != 'edd') {
            $html = '<div class="fc_payment_summary fcrm_purchased_history_customer_summary"><div class="fc_card_header"><h3 class="history_title">' . __('Edd Customer Summary', 'fluentcampaign-pro') . '</h3></div>';
        ...
        $html .= '<div class="fcrm_commerce_purchase_history_widget"><div class="fc_card_header"><h3 class="history_title">' . esc_html__('EDD Purchased Products', 'fluentcampaign-pro') . '</h3></div>';
        ...
        foreach ($commerce->items as $item) {
            ...
            $badges = '<span class="fcrm_product_badge fc_badge_price">' . $sign . number_format_i18n($item->item_value, 2) . '</span>';
            $badges .= '<span class="fcrm_product_badge fc_badge_date"><a target="_blank" rel="noopener" href="' . esc_url($orderUrl) . '">' . date(get_option('date_format'), strtotime($item->created_at)) . '</a></span>';

            $html .= '<li class="fcrm_product_content">';
            $html .= '<span class="fcrm_product_name"><a target="_blank" rel="noopener" href="' . esc_url($orderUrl) . '">' . $productName . '</a></span>';
            $html .= '<span class="fcrm_product_badges">' . $badges . '</span>';
            $html .= '</li>';
        }
        $html .= '</ul></div></div>';
    ...
    /**
     * Sync EDD 3 customers into FluentCRM commerce relation tables.
     *
