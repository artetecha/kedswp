...
abstract class BaseAdvancedReport
{
    ...
    protected function getCustomersByRange($range)
    {
        ...
        foreach ($items as $item) {
            $periodRange[gmdate($format, strtotime($item->date))] = $item->count;
        }
    ...
    protected function getRevenueByRange($range, $productId = false, $itemTypes = [])
    {
        ...
        foreach ($items as $item) {
            $periodRange[gmdate($format, strtotime($item->date))] = $item->revenue;
        }
    ...
    protected function getCompareRange($type, $compareDate, $currentRange)
    {
        ...
        if ($type == 'previous_period') {
            return [
                new \DateTime(gmdate('Y-m-d 00:00:01', $currentRange[0]->getTimestamp() - ($diffDays + 1) * 86400)),
                new \DateTime(gmdate('Y-m-d 23:59:59', $currentRange[0]->getTimestamp() - 86400))
            ];
        } else if ($type == 'previous_month') {
            $fromDate = gmdate('Y-m-d 00:00:01', strtotime($currentRange[0]->format('Y-m-d') . ' -1 month'));
            return [
                new \DateTime($fromDate),
                new \DateTime(gmdate('Y-m-d 23:59:59', strtotime($fromDate) + $diffTimestamps))
            ];
        } else if ($type == 'previous_quarter') {
            $fromDate = gmdate('Y-m-d 00:00:01', strtotime($currentRange[0]->format('Y-m-d') . ' -3 months'));
            return [
                new \DateTime($fromDate),
                new \DateTime(gmdate('Y-m-d 23:59:59', strtotime($fromDate) + $diffTimestamps))
            ];
        } else if ($type == 'previous_year') {
            $fromDate = gmdate('Y-m-d 00:00:01', strtotime($currentRange[0]->format('Y-m-d') . ' -12 months'));
            return [
                new \DateTime($fromDate),
                new \DateTime(gmdate('Y-m-d 23:59:59', strtotime($fromDate) + $diffTimestamps))
            ];
        } else if ($type == 'custom' && $compareDate) {
            if (is_array($compareDate) && count(array_filter($compareDate)) >= 2) {
                return [
                    new \DateTime($compareDate[0] . ' 00:00:01'),
                    new \DateTime($compareDate[1] . ' 23:59:59')
                ];
            }
            $fromDate = (is_array($compareDate) ? $compareDate[0] : $compareDate) . ' 00:00:01';
            return [
                new \DateTime($fromDate),
                new \DateTime(gmdate('Y-m-d 23:59:59', strtotime($fromDate) + $diffTimestamps))
            ];
    ...
    protected function getReportParams($filters)
    {
        ...
        $compareDate = Arr::get($filters, 'compare_range', Arr::get($filters, 'compare_date', []));
        $compareRangeType = Arr::get($filters, 'compare_type', 'previous_period');

        if (!$currentRange || count(array_filter($currentRange)) < 2) {
            $currentRange = [
                new \DateTime(gmdate('Y-m-d 00:00:01', strtotime('-1 months'))),
                new \DateTime(gmdate('Y-m-d 23:59:59'))
            ];
        } else {
            $currentRange = [
    ...
    protected function formatDataSet($items, $title, $compareRange, $type = 'primary')
    {
        ...
        if ($type != 'primary') {
            $data['backgroundColor'] = '#1FC16B';
            $data['borderColor'] = '#1FC16B';
        }
