...
<table class="fce_row fc_latest_post_item <?php echo esc_attr($atts['selectedLayout']); ?>" border="0" cellpadding="0" cellspacing="0" width="100%" style="<?php echo esc_attr($settings['gapStyle']); ?><?php echo esc_attr($settings['itemStyle']); ?>">
    <tbody>
        <?php
        $contentPadding = !empty($settings['isRtl']) ? '20px 0 20px 8px' : '20px 8px 20px 0';
        $imagePadding   = !empty($settings['isRtl']) ? '20px 8px 20px 0' : '20px 0 20px 8px';
        ?>
        <tr class="fc_latest_post_item_tr">
            ...
            <?php if ( get_the_post_thumbnail_url($post) && $atts['showImage'] == true) { ?>
                <td class="fc_latest_post_img" width="220px" height="220px" valign="middle" style="padding: <?php echo esc_attr($imagePadding); ?>;">
                    <a href="<?php echo esc_url($postUrl); ?>" class="fc_latest_post_overlay" style="background: url(<?php echo esc_url(get_the_post_thumbnail_url($post)); ?>) center no-repeat; background-size: <?php echo esc_attr($atts['backgroundType']); ?>;">
                    </a>
                </td>
            <?php } ?>
