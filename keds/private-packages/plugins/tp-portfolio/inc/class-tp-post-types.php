...
if ( ! class_exists( 'TP_PORTFOLIO_POST_TYPES' ) ) {
	...
	class TP_PORTFOLIO_POST_TYPES {
		...
		public function register_taxonomies() {
			...
			// Portfolio Tags
			$labels = array(
				'name'                       => _x( 'Project Tags', 'Taxonomy General Name', 'tp-portfolio' ),
				'singular_name'              => _x( 'Project Tag', 'Taxonomy Singular Name', 'tp-portfolio' ),
				'menu_name'                  => esc_html__( 'Project Tags', 'tp-portfolio' ),
				'all_items'                  => esc_html__( 'All Project Tags', 'tp-portfolio' ),
				'new_item_name'              => esc_html__( 'New Project Tag Name', 'tp-portfolio' ),
				'add_new_item'               => esc_html__( 'Add New Project Tag', 'tp-portfolio' ),
				'edit_item'                  => esc_html__( 'Edit Project Tag', 'tp-portfolio' ),
				'update_item'                => esc_html__( 'Update Project Tag', 'tp-portfolio' ),
				'separate_items_with_commas' => esc_html__( 'Separate Project Tags with commas', 'tp-portfolio' ),
				'search_items'               => esc_html__( 'Search Project Tags', 'tp-portfolio' ),
				'add_or_remove_items'        => esc_html__( 'Add or remove Project Tags', 'tp-portfolio' ),
				'choose_from_most_used'      => esc_html__( 'Choose from the most used Project Tags', 'tp-portfolio' ),
			);
			$args   = array(
				'labels'            => $labels,
				'hierarchical'      => false,
		...
		public function register_metabox( $meta_boxes ) {
			$meta_boxes[] = array(
				...
				'fields' => array(
					...
					array(
						...
						'id'       => 'selectPortfolio',
						'type'     => 'select',
						'options'  => $this->tp_portfolio_type_select(),
 						'multiple' => false,
					...
					array(
						'name'  => esc_html__( 'Video URL or own Embedd Code(Audio Embedd Code is possible, too)', 'tp-portfolio' ),
					...
					array(
						...
						'type'             => 'image',
						'max_file_uploads' => 1,
						'class'            => 'portfolio_type_gallery portfolio_type_image',
						'tab'              => 'Type',
						'icon'             => 'dashicons-layout',
					),
					...
					array(
						'name' => esc_html__( 'Finish date', 'tp-portfolio' ),
						'id'   => 'portfolio_date_end',
						'type' => 'textfield',
						'tab'  => 'Info',
						'desc' => __( 'Date finish project', 'tp-portfolio' ),
						'icon' => 'dashicons-info',
					),

					array(
						'name' => esc_html__( 'File Upload (PDF)', 'tp-portfolio' ),
						'id'   => 'portfolio_file_upload',
						'type' => 'upload',
						'tab'  => 'Info',
						'icon' => 'dashicons-info',
					),
					array(
						'name'   => esc_html__( 'Repeater Data', 'tp-portfolio' ),
			...
			return apply_filters( 'thim_portfolio_meta_boxes', $meta_boxes );
		}

		public function tp_portfolio_type_select() {
			$portfolio_type = apply_filters(
				'tp_portfolio_type_select',
				array(
					...
					'portfolio_pdf'               => __( 'Pdf', 'tp-portfolio' ),
		...
		public function template_include( $template ) {
			...
			} elseif ( get_post_type() == 'portfolio' && is_single() ) {
