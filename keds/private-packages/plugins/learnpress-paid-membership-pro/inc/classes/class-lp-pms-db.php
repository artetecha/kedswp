...
class LP_PMS_DB extends LP_Database {
	 ...
	 * @version 1.0.1
	 */
	public function getCoursesByLevel( int $level_id = 0, array $cols = [ 'ID', 'post_title' ] ) {
		$nameCols = '';
		foreach ( $cols as $col ) {
			if ( empty( $nameCols ) ) {
				$nameCols = 'p.' . $col;
	...
	public function getLastOrderMembershipOfUser( $user_id = 0, $membership_id = 0 ) {
		$query = $this->wpdb->prepare(
			"SELECT p_meta.post_id AS post_id FROM $this->tb_postmeta as p_meta
	 ...
	 * @return array|object|null
	 */
	public function getCourseIdsOnLpOrder( int $lp_order_id = 0 ) {
		$sql = $this->wpdb->prepare(
			"SELECT item_id, order_item_id
			FROM $this->tb_lp_order_items
			WHERE order_id = %d
			AND item_type = %s",
			$lp_order_id,
	...
	/**
	 * Count total courses assigned to level
	 *
	 * @param int $level_id
	 *
	 * @return int
	 ...
	 * @version 1.0.0
	 */
	public function countCoursesOnLevel( int $level_id = 0 ): int {
		$sql = $this->wpdb->prepare(
			"SELECT COUNT(ID) FROM
				(SELECT p.ID FROM $this->tb_posts AS p
					...
					AND pm.meta_value = %d
					AND p.post_type = 'lp_course'
					AND p.post_status = 'publish') AS ID",
			$level_id
		);
