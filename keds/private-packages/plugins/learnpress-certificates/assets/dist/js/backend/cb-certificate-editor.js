...
"use strict";
/*!********************************************************!*\
  !*** ./assets/src/js/backend/cb-certificate-editor.js ***!
  \********************************************************/
__webpack_require__.r(__webpack_exports__);
...
(function () {
  ...
  function fetchCertAJAX(action, data, onSuccess) {
    ...
    window.lpAJAXG.fetchAJAX(Object.assign({
      ...
      error: function (err) {
        ...
        AssetsJsPath_lpToastify__WEBPACK_IMPORTED_MODULE_0__.show(err && err.message || headerData.textRequestFailed || '', 'error');
      }
    });
  }
  ...
  const normalOptions = [{
    ...
    value: 'draft',
    label: badgeData.labelDraft || ''
  }, {
    value: 'pending',
    label: badgeData.labelPending || ''
  }];
  ...
  function saveCertificateAndReload(status, btn) {
    ...
    if (!titleVal.trim()) {
      AssetsJsPath_lpToastify__WEBPACK_IMPORTED_MODULE_0__.show(publishBtnData.textTitleRequired || '', 'error');
      return;
    }
    AssetsJsPath_utils__WEBPACK_IMPORTED_MODULE_1__.lpSetLoadingEl(btn, 1);
    syncDateToHiddenFields();
    let postDate = '';
    const elMm = document.getElementById('mm');
    const elJj = document.getElementById('jj');
