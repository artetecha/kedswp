...
/* harmony export */   argsToastify: () => (/* binding */ argsToastify),
...
"use strict";
/*!*********************************************************!*\
  !*** ./assets/src/js/frontend/cb-certificate-editor.js ***!
  \*********************************************************/
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var AssetsJsPath_lpToastify__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! AssetsJsPath/lpToastify */ "./assets/src/js/lpToastify.js");

(function () {
  const L = window.lpCertCBEditor || {};
  const i18n = L.i18n || {};
  const certId = parseInt(L.cert_id, 10) || 0;
  const backUrl = L.back_url || '';
  const statusLabels = L.status_labels || {};
  const uploadIconUrl = L.upload_icon_url || '';
  const originalPostStatus = L.original_post_status || 'draft';
  ...
  function fetchCertAJAX(action, data, onSuccess) {
    ...
    window.lpAJAXG.fetchAJAX(Object.assign({
      action: action
    }, data), {
      success: function (res) {
        if (res.status === 'success') {
          ...
          AssetsJsPath_lpToastify__WEBPACK_IMPORTED_MODULE_0__.show(res.message || i18n.error || 'Error', 'error');
        }
      },
      error: function (err) {
        console.error('AJAX error:', err);
        AssetsJsPath_lpToastify__WEBPACK_IMPORTED_MODULE_0__.show(err && err.message || i18n.request_failed || 'Request failed', 'error');
      }
    });
  }
  ...
  const normalOptions = [{
    ...
    label: isDateInFuture() ? i18n.scheduled || '' : i18n.published || ''
  }, {
    value: 'draft',
    label: i18n.draft || ''
  }, {
    value: 'pending',
    label: i18n.pending_review || ''
  }];
  function updatePublishOption() {
    ...
    firstOpt.textContent = future ? i18n.scheduled || '' : i18n.published || '';
  ...
  function rebuildStatusSelect(vis) {
    ...
    if (vis === 'private') {
      ...
      opt.value = 'private';
      opt.textContent = i18n.privately_published || '';
      opt.selected = true;
    ...
    normalOptions.forEach(function (item) {
      ...
      if (item.value === selectVal || item.value === 'publish' && selectVal === 'future' || item.value === 'future' && selectVal === 'publish') {
        opt.selected = true;
      }
      cbCertStatus.appendChild(opt);
    });
  ...
  function updateBadge() {
    ...
    badge.className = 'cb-cert-status-badge cb-cert-status-badge--' + st;
    badge.textContent = st === 'private' ? i18n.privately_published || '' : statusLabels[st] || st;
  }
  const publishBtns = document.querySelectorAll('.cb-cert-btn-publish');
  publishBtns.forEach(function (btn) {
    btn.addEventListener('click', function () {
      saveCertificateAndReload(getStatus(), btn);
    });
  });
  function saveCertificateAndReload(status, btn) {
    const title = document.querySelector('.cb-cert-title-input');
    ...
    if (!titleVal.trim()) {
      AssetsJsPath_lpToastify__WEBPACK_IMPORTED_MODULE_0__.show(i18n.title_required || '', 'error');
      return;
    }
    btn.disabled = true;
    ...
    if (elMm && elJj && elAa && elHh && elMn && elSs) {
      postDate = elAa.value + '-' + elMm.value + '-' + elJj.value + ' ' + elHh.value + ':' + elMn.value + ':' + elSs.value;
    }
  ...
  function buildUploadArea() {
    ...
    textP.appendChild(document.createTextNode(' ' + (i18n.or_drag_drop || '')));
    area.appendChild(textP);
    const hintP = document.createElement('p');
    hintP.className = 'upload-hint';
    hintP.textContent = i18n.jpg_png_less_1mb || '';
  ...
  if (headerTrashBtn) {
    headerTrashBtn.addEventListener('click', async function (e) {
      ...
      if (typeof Swal === 'undefined') {
        if (!confirm(i18n.trash_confirm_native || '')) {
          return;
        }
      } else {
        const result = await Swal.fire({
          title: i18n.are_you_sure || '',
          text: i18n.moved_to_trash || '',
          icon: 'warning',
  ...
  const titleInput = document.querySelector('.cb-cert-title-input');
  const titleText = document.querySelector('.cb-cert-title-text');
  if (titleInput && titleText) {
    titleInput.addEventListener('input', function () {
      titleText.textContent = this.value || i18n.placeholder_title || '';
    });
  }
})();
})();
...
/******/ })()
;
//# sourceMappingURL=cb-certificate-editor.js.map