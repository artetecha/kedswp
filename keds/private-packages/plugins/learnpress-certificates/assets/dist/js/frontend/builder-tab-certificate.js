...
"use strict";
/*!***********************************************************!*\
  !*** ./assets/src/js/frontend/builder-tab-certificate.js ***!
  \***********************************************************/
__webpack_require__.r(__webpack_exports__);
...
const handleDuplicate = el => {
  ...
  window.lpCertConfirm(swalFromEl(el)).then(result => {
    if (result.isConfirmed) {
      sendAjax(el, 'duplicate_certificate', {
...
const handleChangeStatus = (el, status) => {
  ...
  if (status === 'trash') {
    window.lpCertConfirm(swalFromEl(el)).then(result => {
      if (result.isConfirmed) {
        sendAjax(el, 'change_status_certificate', {
          certificate_id: certId,
          status
        });
      }
...
const sendAjax = (el, action, extraData = {}) => {
  ...
  const callBack = {
    ...
    error: error => {
      AssetsJsPath_lpToastify__WEBPACK_IMPORTED_MODULE_0__.show(error.message || error, 'error');
    },
    completed: () => {
      setLoading(el, false);
    }
...
/******/ })()
;
//# sourceMappingURL=builder-tab-certificate.js.map