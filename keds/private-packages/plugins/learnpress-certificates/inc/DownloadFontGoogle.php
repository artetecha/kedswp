...
class DownloadFontGoogle {
	...
	private function get_fonts_from_font_face( $font_face ) {
		...
		$font_weight = $matches[1] ?? '400';

		// get font-style.
		$pattern = '/font-style:\s*(.*);/i';
		preg_match( $pattern, $font_face, $matches );

		$font_style = $matches[1] ?? 'normal';

		// get font-display.
		$pattern = '/font-display:\s*(.*);/i';
		preg_match( $pattern, $font_face, $matches );

		$font_display = $matches[1] ?? 'swap';
		...
		$font_src    = $matches[1] ?? '';
		$font_format = $matches[2] ?? 'woff2';

		if ( empty( $font_src ) || empty( $font_family ) ) {
			return array();
		}

		$parsed_url = wp_parse_url( $font_src );
		$path_parts = explode( '/', $parsed_url['path'] ?? '' );

		$folder_name = isset( $path_parts[2] ) ? $path_parts[2] : sanitize_file_name( strtolower( str_replace( ' ', '', $font_family ) ) );

		$query_hash   = ! empty( $parsed_url['query'] ) ? substr( md5( $parsed_url['query'] ), 0, 8 ) : '';
		$clean_family = sanitize_file_name( strtolower( str_replace( ' ', '', $font_family ) ) );
		$file_name    = $clean_family . '-' . $font_weight . '-' . $font_style;

		if ( $query_hash ) {
			$file_name .= '-' . $query_hash;
		}
