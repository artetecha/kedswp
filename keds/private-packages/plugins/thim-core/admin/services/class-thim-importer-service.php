	...
	public static function revslider( $revsliders ) {
		try {
			...
			if ( $prefix_folder_data_demo ) {
				...
				$dir_revsliders = apply_filters( 'thim_core_importer_directory_revsliders', get_template_directory() . '/inc/data/revsliders/' );
			}
			...
			foreach ( $revsliders as $file ) {
				...
				if ( defined( 'RS_REVISION' ) ) {
					if ( version_compare( RS_REVISION, '7.0', '<' ) ) {
