...
class Thim_Child_Themes extends Thim_Admin_Sub_Page {
	...
	public function handle_ajax_action() {
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'thim-child-themes' ) ) {
			wp_send_json_error( __( 'Invalid nonce.', 'thim-core' ) );
		}
		$slug   = isset( $_POST['slug'] ) ? sanitize_key( $_POST['slug'] ) : false;
		$action = isset( $_POST['thim_action'] ) ? sanitize_key( $_POST['thim_action'] ) : false;
	...
	private function get_data_template() {
		...
		return array(
			...
			'nonce'       => wp_create_nonce( 'thim-child-themes' ),
		);
	}
}
