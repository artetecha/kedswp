...
class Thim_Plugins_Manager extends Thim_Admin_Sub_Page {
	...
	public static function get_link_download_plugin( $slug ) {
		...
		if ( $purchase_token ) {
			$url = Thim_Admin_Config::get( 'api_thim_market' ) . '/license/download';
			$url = add_query_arg(
				array(
					'slug'      => $slug,
	...
	public function handle_ajax_plugin_request_actions() {
		if ( ! isset( $_POST['nonce'] ) ||  ! wp_verify_nonce( $_POST['nonce'], 'thim-plugins' ) ) {
			wp_send_json_error( [ 'messages' => [ __('Invalid nonce.', 'thim-core') ] ] );
		}
	...
	public function localize_script() {
		...
		wp_localize_script(
			...
			array(
				...
				'nonce'             => wp_create_nonce( 'thim-plugins' ),
			)
		);
	}
