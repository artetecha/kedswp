...
class Thim_Getting_Started extends Thim_Admin_Sub_Page {
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script(
			'thim-importer',
			THIM_CORE_ADMIN_URI . '/assets/js/importer/importer' . $extend,
			array(
				'wp-util',
				'jquery',
				'backbone',
		...
		wp_enqueue_script(
			...
			THIM_CORE_ADMIN_URI . '/assets/js/getting-started/getting-started-v2' . $extend,
			array(
				'thim-plugins',
				'thim-importer',
				'thim-video-youtube',
	...
	private function localize_script() {
		wp_localize_script(
			...
			array(
				...
				'steps'    => self::get_steps(),
				'nonce'    => wp_create_nonce( 'thim-getting-started' ),
			)
		);
	...
	public function handle_ajax() {
		if ( ! isset( $_REQUEST['nonce'] ) || ! wp_verify_nonce( $_REQUEST['nonce'], 'thim-getting-started' ) ) {
			wp_send_json_error( array( 'messages' => array( __( 'Invalid nonce.', 'thim-core' ) ) ) );
		}
	...
	public function render_step_template( $slug ) {
		...
		return Thim_Template_Helper::template(
			'dashboard/gs-steps/master.php',
			array(
				'slug' => $slug,
				'html' => $html,
			),
