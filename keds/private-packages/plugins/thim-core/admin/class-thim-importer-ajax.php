...
class Thim_Importer_AJAX {
	...
	public function handle_ajax() {
		...
		try {
			...
			$packages = array_map( 'sanitize_key', (array) $_POST['packages'] );
	...
	private function _step_by_step( $step = '' ) {
		...
		$callable_name = is_array( $callable ) ? implode( '::', $callable ) : (string) $callable;
		// translators: %s: function name.
		throw Thim_Error::create( sprintf( __( 'Not found function: %s', 'thim-core' ), $callable_name ), 3 );
	}
