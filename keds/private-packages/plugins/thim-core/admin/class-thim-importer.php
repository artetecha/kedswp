...
class Thim_Importer extends Thim_Admin_Sub_Page {
	...
	public function reset_import_demo() {
		...
		check_admin_referer( 'thim_reset_importer' );
	...
	public function handle_ajax() {
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'thim-importer' ) ) {
			wp_send_json_error( __( 'Invalid nonce.', 'thim-core' ) );
		}

		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error( __( 'Permission denied.', 'thim-core' ) );
		}
