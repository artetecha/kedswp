...
class Typography extends Field {
	...
	protected $control_class = '\ThimPress\Customizer\Control\Typography';
	...
	private function add_sub_fields( $args ) {
		...
		// Responsive: a single device switcher at the group level controls font-size & line-height.
		$is_responsive = isset( $args['choices']['responsive'] ) && true === $args['choices']['responsive'];

		$generic_choices = array(
			'type'        => 'hidden',
			'parent_type' => 'thim-typography',
		);
		if ( $is_responsive ) {
			$generic_choices['responsive'] = true;
		}
			...
			new \ThimPress\Customizer\Field\Dimension(
				wp_parse_args(
					array(
						...
						'choices'       => $is_responsive
							? array(
								'responsive'       => true,
								'responsive_group' => $args['id'],
							)
							: array(),
						'css_vars'      => array(),
						'output'        => array(),
	...
	private function get_font_family_choices() {
		...
		// Always return the grouped structure for the React component.
		return $choices;
	...
	public function filter_control_args( $args, $wp_customize ) {
		...
		$args['choices'] = array_merge(
			...
			array(
				...
				'fonts'      => $this->get_font_family_choices(),
				'variants'   => $this->get_variant_choices(),
			)
		);
