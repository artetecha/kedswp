...
class UserAssignmentModel extends UserItemModel {
	...
	public function upload_files_of_student( $files ) {
		...
		foreach ( $files['name'] as $key => $value ) {
			...
			if ( ! in_array( $file_ext, $file_extends_allow ) ) {
				throw new Exception(
					sprintf( esc_html__( 'File %s type is invalid!', 'learnpress-assignments' ), $file_name )
				);
			}
	...
	public function can_finish_course() {
		...
		try {
			...
			if ( ! $courseModel ) {
				throw new Exception( __( 'Course not exists!', 'learnpress-assignments' ) );
			}
			...
			if ( ! $userCourseModel->has_enrolled() ) {
				throw new Exception( __( 'Course is not enroll!', 'learnpress-assignments' ) );
			}
			...
			if ( ! $course_results['pass'] ) {
				...
				if ( $allow_finish_when_all_item_completed ) {
					...
					if ( $course_total_items_obj && $course_results['completed_items'] < (int) $course_total_items_obj->count_items ) {
						throw new Exception( __( 'You must complete all items in course', 'learnpress-assignments' ) );
					}
				} else {
					throw new Exception( __( 'You must passed course', 'learnpress-assignments' ) );
				}
			}
		} catch ( Throwable $e ) {
