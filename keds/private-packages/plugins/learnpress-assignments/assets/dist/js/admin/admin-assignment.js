...
"use strict";
/*!*************************************************!*\
  !*** ./assets/src/js/admin/admin-assignment.js ***!
  \*************************************************/
__webpack_require__.r(__webpack_exports__);
...
( function ( $ ) {
	function _ready() {
		$( 'input[type=radio][name=_lp_course_result]' ).on(
			'change',
			function ( e ) {
		...
		$( '.lp-metabox-evaluate-radio' ).on( 'click', function ( e ) {
			...
			$.ajax( {
				...
				type: 'POST',
				data: {
					action: 'lp_assignment_get_passing_grade_final_assignment',
					course_id: courseId,
				},
		...
		if ( $( '.lp-meta-box--assignments' ).length > 0 ) {
			$( '#_lp_mark' ).on( 'change', function () {
				$( '._lp_mark_field' )
					.find( '.learn-press-tip-floating' )
					.remove();
				const maxMark = parseFloat( $( '#_lp_mark' ).val() );
				const passGrade = parseFloat( $( '#_lp_passing_grade' ).val() );
				if ( maxMark < passGrade ) {
					$( '._lp_mark_field' ).append(
						'<div class="learn-press-tip-floating">' +
							lpAssignment.invalid_mark +
							'</div>'
					);
					$( '#_lp_mark' ).val( passGrade );
				}
			} );
			$( '#_lp_passing_grade' ).on(
				'change keydown paste input',
				function () {
					$( '._lp_passing_grade_field' )
						.find( '.learn-press-tip-floating' )
						.remove();
					const maxMark = parseFloat( $( '#_lp_mark' ).val() );
					const passGrade = parseFloat(
						$( '#_lp_passing_grade' ).val()
					);
					if ( passGrade > maxMark ) {
						$( '._lp_passing_grade_field' ).append(
							'<div class="learn-press-tip-floating">' +
								lpAssignment.invalid_mark_grade +
								'</div>'
						);
...
document.addEventListener( 'click', ( e ) => {
	...
	if ( target.classList.contains( 'lp-btn-action-instructor-assignment' ) ) {
		const form = target.closest( 'form' );
		if ( ! form ) {
			return;
		}

		const elInputAction = form.querySelector( 'input[name="action"]' );
		if ( ! elInputAction ) {
			return;
		}

		elInputAction.value = target.value;
		form.submit();
	} else if ( target.closest( '.assignment-students-actions' ) ) {
		const elActions = target.closest( '.assignment-students-actions' );
		const userId = elActions.dataset.user_id;
		const courseId = elActions.dataset.course_id;
		const assignmentId = elActions.dataset.assignment_id;
		const elA = target.closest( 'a' );
		const action = elA.dataset.action;
		if ( ! action ) {
			return;
		}
		...
		let messageConfirm = lpAssignment[ action ];
		...
		if ( confirm( messageConfirm ) ) {
			...
			toastifyAssignment.options.className = 'lp-toast';
			...
			const callBack = {
				success: function ( response ) {
					...
					if (
						[
							'lp_assignment_delete_submission',
							'lp_assignment_re_evaluate',
						].indexOf( action ) !== -1
					) {
						setTimeout( () => {
				...
				error: function ( err ) {
					...
					toastifyAssignment.options.text = err;
					toastifyAssignment.showToast();
				},
				complete: function () {},
			};
			(0,_utils_js__WEBPACK_IMPORTED_MODULE_0__.lpFetchAPI)( ajaxurl, dataSend, callBack );
		}
	}
} );

// Add label to WPEditor for Admin can understand type is description task.
(0,_utils_js__WEBPACK_IMPORTED_MODULE_0__.lpOnElementReady)( '#wp-content-editor-tools', ( elAddMediaButton ) => {
	if ( lpDataAdmin.current_screen !== 'lp_assignment' ) {
		return;
	}

	elAddMediaButton.insertAdjacentHTML(
		'afterbegin',
		`<div style="margin-bottom: 10px; font-size: 1rem"><strong>${ lpAssignment.label_edit_task }</strong></div>`
	);
} );
...
/******/ })()
;
//# sourceMappingURL=admin-assignment.js.map