...
document.addEventListener( 'submit', ( e ) => {
	...
	if ( target.classList.contains( 'lp-form-start-assignment' ) ) {
	...
	// Event start assignment
	if ( target.classList.contains( 'lp-form-retake-assignment' ) ) {
		const btnSubmit = target.querySelector( 'button[type="submit"]' );
		(0,_utils_js__WEBPACK_IMPORTED_MODULE_0__.lpSetLoadingEl)( btnSubmit, 1 );
	}
} );
...
document.addEventListener( 'click', ( e ) => {
	...
	if (
		...
		const elAction = form.querySelector( 'input[name=lp-load-ajax]' );
		if ( ! elAction ) {
			return;
		}
		...
		if ( action === 'assignment_submit_answer' ) {
