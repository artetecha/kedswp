...
import { packageSelection, reset } from '@/store/package';
import useMatchMutate from '@/hooks/useMatchMutate';
import { useDispatch, useSelector } from 'react-redux';

export default function AddNewPakage( { popup, setPopup } ) {
	...
	const onSave = async () => {
		...
		try {
			if ( packageSlice?.courses?.length === 0 ) {
				throw new Error( __( 'Please select at least one course' ) );
			}

			const response = await apiFetch( {
				path: '/learnpress-package/v1/admin/package/0',
				method: 'POST',
				data: { ...packageSlice },
			} );
