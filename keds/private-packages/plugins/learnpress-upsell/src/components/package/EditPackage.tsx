...
export default function EditPackage( { packageId, setPackageId }: Props ) {
	...
	const { data, error, isLoading } = useSWR(
		`/learnpress-package/v1/admin/package/${ packageId || 0 }`,
	...
	useEffect( () => {
		if ( data?.success ) {
			dispatch( changeValue( { ...data.package } ) );
		}
	}, [ data ] );

	return (
		<Transition appear show={ !! packageId }>
			<Dialog
				open={ true }
				as="div"
				className="fixed inset-0 z-[99999] overflow-y-auto"
				onClose={ () => setPackageId( 0 ) }
			>
							...
							<div className="flex flex-col flex-1 h-full">
								...
								<div className="sticky bottom-0 bg-white border-t border-x-0 border-b-0 border-gray-200 border-solid flex items-center gap-x-2 pt-5 pb-3">
									<button
										...
										onClick={ () => packageSlice?.courses?.length === 0 ? addToast( 'Please select at least one course', 'error' ) : trigger( { ...packageSlice } ) }
