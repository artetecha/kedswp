...
use LP_Woo_Assign_Course_To_Product;
use Throwable;
use WC_Order;
use WC_Order_Item_LP_Course;
use WC_Order_Item_Product;

/**
 * Class LPWooOrderHandler
 *
...
class LPWooOrderHandler {
	...
	public static $KEY_META_ID = '_learn_press_order_id';
	...
	public function create_lp_order() {
		try {
			...
			$lp_order_id = $wc_order->get_meta( self::$KEY_META_ID );
			...
			$order_total      = 0;
			$order_subtotal   = 0;
			$opt_buy_course   = WooGateway::is_by_courses_via_product();
			...
			$lp_order = learn_press_get_order( $lp_order_id );
			$wc_order->update_meta_data( self::$KEY_META_ID, $lp_order_id );
	...
	public function add_item_to_order( array $lp_order_items = array() ) {
		try {
			...
			/*
			 * Check the WooCommerce order status after LP order is created. If it's 'processing' or 'completed', mark the LP order as 'completed'.
			 */
			if ( in_array( $wc_order->get_status(), array( 'processing', 'completed' ) ) ) {
				$lp_status = 'lp-completed';
			} else {
