...
if ( $item_name == $order->get_id() ) {
	$item_name = sprintf( __( 'Order %s on', 'learnpress-2checkout-payment' ), $order_key, $sitename );
}
...
if ( $settings['test_mode'] == 'yes' ) {
	$input_demo = "<input type='hidden' name='demo' value='Y' />";
}
$url               = get_site_url() . '/?' . learn_press_get_web_hook( '2checkout' ) . '=1';
$merchant_order_id = $order->get_id() . '-' . $order_key;
...
?>
	...
	<div>
		<form name="checkout_payment_form"
				id="learn-press-2checkout-payment-form"
				action='<?php echo esc_attr( $action ); ?>'
				method='post'>
			<?php echo $input_demo; ?>
			<input type='hidden' name='sid' value='<?php echo esc_attr( $settings['sid'] ); ?>'>
			<input type='hidden' name='mode' value='2CO'>

			<input type='hidden' name='li_0_type' value='product'>
			<input type='hidden' name='li_0_name' value='<?php echo esc_attr( $item_name ); ?>'>
			<input type='hidden' name='li_0_product_id' value='<?php echo esc_attr( $merchant_order_id ); ?>'>
