...
class Thim_Ekit_Widget_List_Instructors extends Widget_Base {
	...
	protected function _register_setting_slider() {
		...
		$this->add_control(
			'pause_on_hover',
			[
				'label'              => esc_html__( 'Pause on Hover', 'eduma' ),
				'type'               => Controls_Manager::SWITCHER,
				'default'            => 'yes',
				'label_on'           => esc_html__( 'Yes', 'eduma' ),
				'label_off'          => esc_html__( 'No', 'eduma' ),
			...
			<div class="<?php echo esc_attr( $class ); ?>">
				<div class="<?php echo esc_attr( $class_inner ); ?>">
					...
					foreach ( $panel_list as $key => $panel ) {
						...
						echo '<h4 class="instructor-name"><a href="' . esc_url( $link_author ) . '">' . $instructor->get_display_name() . '</a></h4>';
						...
						echo '</div>';
						echo '</div>';
						echo '<div class="instructor-view-profile"><a href="' . esc_url( $link_author ) . '">' . esc_html__( 'View portfolio', 'eduma' ) . '</a></div>';
						echo '</div>';
						echo '</div>';
					}
					?>
