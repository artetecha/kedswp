...
if ( $archive_layout ) {
	...
	$template      = get_theme_mod( 'thim_archive_cate_template', 'list' );
	$class_archive = ' blog-switch-layout blog-' . $template;
	if ( $template == 'grid' ) {
		$layout_type = 'grid';
	} elseif ( $template == 'list' && get_theme_mod( 'thim_feature_image_pos', 'beside' ) == 'above' ) {
		$class_archive = ' blog-switch-layout blog-default';
	}
}
