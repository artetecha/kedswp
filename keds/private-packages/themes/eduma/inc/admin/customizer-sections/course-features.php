...
thim_customizer()->add_field(
	array(
		...
		'priority' => 13,
		'choices'  => array(
			'on'  => esc_html__( 'Enable', 'eduma' ),
			'off' => esc_html__( 'Disable', 'eduma' ),
		),
	)
);
...
if ( class_exists( '\Thim_EL_Kit\Functions' ) ) {
	$loop_course_item = array( '' => esc_html__( 'Default', 'eduma' ) ) + \Thim_EL_Kit\Functions::instance()->get_pages_loop_item( 'lp_course' );
	$hiden_class      = '';
} else {
	$loop_course_item = array( '' => esc_html__( 'Default', 'eduma' ) );
	$hiden_class      = ' hidden';
}
