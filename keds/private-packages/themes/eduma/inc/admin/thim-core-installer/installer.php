...
if ( ! class_exists( 'Thim_Core_Installer' ) ) {
	...
	class Thim_Core_Installer {
		...
		private function setup_environments() {
				...
				<tr>
					...
					<td>
						...
						if ( $args['memory_limit'] < 134217728 ) {
							echo '<mark class="warning">' . sprintf( __( '<strong>%1$s</strong> - We recommend setting memory to at least <strong>128MB</strong>. To learn how, see: <a href="%2$s" target="_blank">Increasing memory allocated to PHP.</a>', 'eduma' ), size_format( $args['memory_limit'] ), 'http://codex.wordpress.org/Editing_wp-config.php#Increasing_memory_allocated_to_PHP' ) . '</mark>';
						} else {
			...
			<div class="notice notice-error">
				<p><?php printf( __( 'Please follow those instructions above to make sure your server is ready to use %1$s theme. If you need assistance, please get our support <a href="%2$s" target="_blank">here</a>.', 'eduma' ), $theme['name'], $theme['support'] ); ?></p>
			</div>
			<?php
		}
