			...
			<div class="comment-right">
				...
				<div class="comment-extra-info">
					...
					<div class="date" itemprop="commentTime">
						<?php echo esc_html( get_comment_date() ) . ' ' . esc_html( get_comment_time() ); ?>
					</div>
...
function thim_excerpt_length() {
	return get_theme_mod( 'thim_archive_excerpt_length', 30 );
}
			...
			foreach ( $sharings as $sharing ) {
				switch ( $sharing ) {
					...
					case 'twitter':
						...
						break;
					case 'pinterest':
						echo '<li><div class="pinterest-social"><a target="_blank" class="pinterest"  href="http://pinterest.com/pin/create/button/?url=' . urlencode( get_permalink() ) . '&amp;description=' . rawurlencode( esc_attr( get_the_excerpt() ) ) . '&amp;media=' . urlencode( wp_get_attachment_url( get_post_thumbnail_id() ) ) . '" onclick="window.open(this.href); return false;" title="' . esc_attr__( 'Pinterest', 'eduma' ) . '"><i class="edu-pinterest-p"></i></a></div></li>';
						break;
					case 'linkedin':
						echo '<li><div class="linkedin-social"><a target="_blank" class="linkedin" href="https://www.linkedin.com/shareArticle?mini=true&url=' . urlencode( get_permalink() ) . '&title=' . rawurlencode( esc_attr( get_the_title() ) ) . '&summary=&source=' . rawurlencode( esc_attr( get_the_excerpt() ) ) . '"><i class="edu-linkedin"></i></a></div></li>';
						break;
				}
			}
...
if ( ! function_exists( 'thim_new_comment_fields' ) ) {
	function thim_new_comment_fields( $fields ) {
		...
		$fields = array(
			'author' => '<p class="comment-form-author">' . '<input placeholder="' . esc_attr__( 'Name', 'eduma' ) . ( $req ? ' *' : '' ) . '" id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30" ' . $aria_req . ' aria-label="' . esc_attr__( 'Name', 'eduma' ) . ( $req ? ' *' : '' ) . '" /></p>',
			'email'  => '<p class="comment-form-email">' . '<input placeholder="' . esc_attr__( 'Email', 'eduma' ) . ( $req ? ' *' : '' ) . '" id="email" name="email" type="text" value="' . esc_attr( $commenter['comment_author_email'] ) . '" size="30" ' . $aria_req . ' aria-label="' . esc_attr__( 'Email', 'eduma' ) . ( $req ? ' *' : '' ) . '" /></p>',
			'url'    => '<p class="comment-form-url">' . '<input placeholder="' . esc_attr__( 'Website', 'eduma' ) . ( $req ? ' *' : '' ) . '" id="url" name="url" type="text" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" ' . $aria_req . ' aria-label="' . esc_attr__( 'Website', 'eduma' ) . ( $req ? ' *' : '' ) . '" /></p>',
		);
...
if ( ! function_exists( 'thim_body_classes' ) ) {
	function thim_body_classes( $classes ) {
		...
		if ( get_theme_mod( 'thim_body_custom_class', false ) ) {
			$classes[] = sanitize_html_class( get_theme_mod( 'thim_body_custom_class', false ) );
		}
		...
		// Fix before loader
		if ( ( get_theme_mod( 'thim_header_sticky', false ) && ! is_singular( 'lp_course' ) ) || thim_eduma_header_position() == 'header_overlay' ) {
			$classes[] = 'fixloader';
		}
		$classes[] = 'nofixcss';
		...
		if ( is_single() && $get_post_type == 'tp_event' && get_theme_mod( 'thim_single_event_style', '' ) == 'style_1' ) {
			$classes[] = 'single-tp-event-2';
		}
		if ( is_404() ) {
			if ( ! get_theme_mod( 'show_title_404', true ) ) {
				$classes[] = 'page-404-hidden';
			}
		}
			...
			<ul class="row">
				...
				foreach ( $port_post as $post ) :
					...
					if ( $bk_ef == '' ) {
						...
						$bk_ef = get_post_meta( $post->ID, 'thim_portfolio_bg_color_ef', true );
						$bg    = 'style="background-color:' . esc_attr( $bk_ef ) . ';"';
					}
					?>
					<li class="col-sm-4">
						<?php
						...
						echo '<h3><a href="' . esc_url( get_permalink( $post->ID ) ) . '" title="' . esc_attr( get_the_title( $post->ID ) ) . '" >' . esc_html( get_the_title( $post->ID ) ) . '</a></h3>';
...
if ( ! function_exists( 'thim_gallery_popup' ) ) {
	function thim_gallery_popup() {
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'thim_gallery_popup_nonce' ) ) {
		...
		if ( $format == 'video' ) {
			...
			if ( filter_var( $url_video, FILTER_VALIDATE_URL ) ) {
				$oembed = wp_oembed_get( $url_video );
				if ( $oembed ) {
					echo '<div class="video">' . $oembed . '</div>';
				}
			} else {
				echo '<div class="video">' . esc_html( $url_video ) . '</div>';
			}
		} else {
			...
			if ( ! empty( $images ) ) {
				foreach ( $images as $k => $value ) {
					...
					if ( $url_image && $url_image != '' ) {
						echo '<a href="' . esc_url( $url_image ) . '">';
						echo '<img src="' . esc_url( $url_image ) . '" />';
						echo '</a>';
						$error = false;
					}
				}
			...
			if ( $error ) {
				if ( is_user_logged_in() ) {
					echo '<div class="thim-gallery-message"><a class="link" href="' . esc_url( $link ) . '">' . esc_html__( 'This post doesn\'t have any gallery images, please add some!', 'eduma' ) . '</a></div>';
				} else {
...
// AJAX Load More for Gallery Posts
add_action( 'wp_ajax_thim_gallery_load_more', 'thim_gallery_load_more' );
add_action( 'wp_ajax_nopriv_thim_gallery_load_more', 'thim_gallery_load_more' );
/**
 * AJAX Load More for Gallery Posts
 */
if ( ! function_exists( 'thim_gallery_load_more' ) ) {
	function thim_gallery_load_more() {
		// Verify nonce for security
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'thim_gallery_nonce' ) ) {
			wp_send_json_error( 'Security check failed', 403 );
		}
		...
		if ( $posts_display->have_posts() ) {
			while ( $posts_display->have_posts() ) :
				...
				if ( $template === 'isotope' ) {
					...
					$html .= '<i class="edu-arrow-right"></i>';
					$html .= '</div></div>';
					$html .= $img;
					$html .= '</a>';
				} else {
		...
		wp_send_json_success(
			array(
				'html'     => $html,
				'has_more' => ( $paged < $posts_display->max_num_pages ),
			)
		);
	}
}
		...
		if ( $args['dropdown'] ) {
			...
			foreach ( $languages as $language ) {
				if ( pll_current_language() == $language->slug ) {
					$current_language = '<a class="lang-item active" href="' . esc_url( $language->home_url ) . '"><img src="' . esc_url( $language->flag_url ) . '" alt="' . esc_attr( $language->slug ) . '" />' . esc_attr( $language->name ) . '</a>';
				}
				$list .= '<li class="lang-item"><a class="lang-select" href="' . esc_url( $language->home_url ) . '"><img src="' . esc_url( $language->flag_url ) . '" alt="' . esc_attr( $language->slug ) . '" />' . esc_attr( $language->name ) . '</a></li>';
			}
...
if ( ! function_exists( 'thim_check_courses_page' ) ) {
	function thim_check_courses_page() {
		if ( function_exists( 'learn_press_get_page_id' ) && ( is_post_type_archive( 'lp_course' ) || ( learn_press_get_page_id( 'courses' ) && is_page( learn_press_get_page_id( 'courses' ) ) ) ) ) {
			return true;
		} else {
			return false;
		}
...
if ( ! function_exists( 'thim_add_marketing_code' ) ) {
	function thim_add_marketing_code() {
		...
		// working with enable thim-core cookie consent option
		$ck_consent = get_option( 'thim_cookie_enable_popup', '' );

		if ( $ck_consent === 'on' ) {
			$script_attr = 'type=text/plain data-thimcookie-category=analytics';
		} else {
			$script_attr = '';
		}
		...
		if ( ! empty( get_theme_mod( 'thim_google_analytics', '' ) ) ) {
			?>
			<script <?php echo esc_attr( $script_attr ); ?>>
			...
			<div class="container">
				...
				if ( ( class_exists( 'WPSEO' ) || class_exists( 'WPSEO_Premium' ) ) && $wpseo['breadcrumbs-enable'] && function_exists( 'yoast_breadcrumb' ) ) {
					...
					if ( thim_use_bbpress() ) {
						bbp_breadcrumb(
							array(
								'before'       => '<ul class="breadcrumbs" id="breadcrumbs">',
								'after'        => '</ul>',
								'sep'          => '</li>',
								'pad_sep'      => false,
								'sep_before'   => '',
...
if ( ! function_exists( 'thim_get_page_title' ) ) {
	function thim_get_page_title( $custom_title, $front_title ) {
		...
		if ( is_post_type_archive() ) {
			...
			$blog_page_title = $blog_page_id ? get_the_title( $blog_page_id ) : esc_html__( 'Blog', 'eduma' );
			
			$heading_title = ! empty( $front_title ) ? $front_title : $blog_page_title;
		}
			...
			<div id="preload">
				...
				if ( $thim_preload_image && $enable_preload == 'image' ) {
					if ( is_numeric( $thim_preload_image ) ) {
						...
						echo '<img src="' . esc_url( $thim_preload_image ) . '" alt="' . esc_html__( 'Preaload Image', 'eduma' ) . '"/>';
					}
				} else {
...
if ( ! function_exists( 'thim_back_to_top' ) ) {
	function thim_back_to_top() {
		if ( get_theme_mod( 'thim_show_to_top', false ) ) {
			...
			echo '<div class="' . $class . '" id="back-to-top">' . $svg_html . '<i class="edu-chevron-up" aria-hidden="true"></i></div>';
		}
	}
}
				...
				<div class="container">
					<div class="copyright-content">
						<div class="row">
							...
							echo '<div class="' . esc_attr( $class_copyright ) . '"><p class="text-copyright">' . wp_kses_post( $copyright_text ) . '</p></div>';
...
if ( ! function_exists( 'thim_footer_class' ) ) {
	function thim_footer_class() {
		$style_header    = get_theme_mod( 'thim_header_style', 'header_v1' );
...
function eduma_lp_get_popular_courses( $limit = 10 ) {
	...
	$result = wp_cache_get( 'lp-popular-courses' );
	...
	wp_cache_set( 'lp-popular-courses', $result, '', $time_cache );
...
if ( ! function_exists( 'list_item_course_cat' ) ) {
	function list_item_course_cat( $course_id, $return = false ) {
		$html  = '';
		...
		if ( $terms && ! is_wp_error( $terms ) ) {
			...
			foreach ( $terms as $term ) {
				...
				$color          = ( isset( $sub_color_cate ) && ! empty( $sub_color_cate ) ) ? esc_attr( sanitize_hex_color( $sub_color_cate ) ) : '';
				$style          = $color ? ' style="color:' . $color . '; border-color:' . $color . '"' : '';
				$html          .= '<a href="' . esc_url( get_term_link( $term->slug, 'course_category' ) ) . '" class="cat-item"' . $style . '>' . esc_html( $term->name ) . '</a>';
			}
			$html .= '</div>';
		}
		if ( $return ) {
...
if ( ! function_exists( 'thim_message_before_importer' ) ) {
	function thim_message_before_importer() {
		...
		echo '<div class="thim-message-import"><h2>' . esc_html__( $title, 'eduma' ) . '</h2></div>';
	}
}
add_filter( 'thim-message-before-importer', 'thim_message_before_importer' );

add_filter(
	'thim_breadcrumb_text_home',
	function () {
...
add_filter( 'thim_get_var_css_customizer', 'thim_custom_internal_css', 30 );
function thim_custom_internal_css( $output_css ) {
	ob_start();
	$base_directory      = thim_eduma_child_locate_template();
	...
	if ( get_theme_mod( 'thim_body_custom_class', false ) == 'demo-yoga' ) {
		echo "\r\n/** CSS Demo Yoga */\r\n";
		include_once THIM_DIR . 'assets/css/libs/demo-yoga.css';
	}
	if ( strpos( get_theme_mod( 'thim_body_custom_class', false ), 'demo-preschool' ) !== false ) {
		echo "\r\n/** CSS Demo Preschool */\r\n";
		include_once THIM_DIR . 'assets/css/libs/demo-preschool.css';
	}
	
	if ( strpos( get_theme_mod( 'thim_body_custom_class', false ), 'demo-global-university' ) !== false ) {
		echo "\r\n/** CSS Demo Global University */\r\n";
		include_once THIM_DIR . 'assets/css/libs/demo-global-university.css';
	}
...
if ( ! function_exists( 'thim_event_nav' ) ) {
	function thim_event_nav() {
		...
		// Previous event (most recent before current)
		...
		$prev_posts = get_posts( $prev_args );
		$prev_post  = ! empty( $prev_posts ) ? $prev_posts[0] : false;

		// Next event (earliest after current)
		...
		$next_post  = ! empty( $next_posts ) ? $next_posts[0] : false;
		...
		<div class="entry-navigation-post">
			<?php if ( $prev_post ) : ?>
				<div class="prev-post">
					<div class="content">
						<p class="heading"><i class="edu-arrow-left"></i><?php echo esc_html__( 'Previous Event', 'eduma' ); ?></p>
						<h5 class="title">
			...
			<?php if ( $next_post ) : ?>
				<div class="next-post">
					<div class="content">
						<p class="heading"><?php echo esc_html__( 'Next Event', 'eduma' ); ?><i class="edu-arrow-right"></i></p>
						<h5 class="title">
...
if ( ! function_exists( 'thim_ajax_load_content_post' ) ) {
	function thim_ajax_load_content_post() {
		check_ajax_referer( 'thim_load_content_post_nonce', 'nonce' );

		ob_start();

		$params = json_decode( str_replace( '\\', '', sanitize_textarea_field( wp_unslash( $_POST['params'] ?? '' ) ) ), true );
		...
		$settings        = $widget_settings->get_widget_settings(
