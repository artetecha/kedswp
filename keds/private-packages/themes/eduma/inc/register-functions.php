		...
		if ( $error_code ) {
			...
			if ( $errors->get_error_messages( 'email_exists' ) ) {
				$error_msg = sprintf(
					...
					__( '<strong>Error:</strong> This email address is already registered. <a href="%s">Log in</a> with this address or choose another one.', 'eduma' ),
					thim_get_login_page_url()
				);
			} else {
	...
	public function validate_password_reset() {
		...
		if ( 'POST' == sanitize_text_field( $_SERVER['REQUEST_METHOD'] ) ) {
			...
			if ( isset( $_POST['password'] ) ) {
				...
				reset_password( $user, sanitize_text_field( wp_unslash( $_POST['password'] ) ) );
