...
if ( $the_query->have_posts() ) :
	...
	<div class="thim-widget-courses-wrapper<?php echo esc_attr( $extra_class ); ?>">
		...
		if ( $view_all_course && 'top' == $view_all_position ) {
			echo '<a class="view-all-courses position-top" href="' . esc_url( get_post_type_archive_link( 'lp_course' ) ) . '">' . esc_attr( $view_all_course ) . '</a>';
		}
		?>
						...
						<div class="thim-course-content">
							...
							<?php else : ?>
								<div class="course-meta">
									<?php
									$student_text = $total_lessson = ''; // phpcs:ignore
									...
									if ( $hidden_author == 'no' ) {
										...
										if ( $course ) {
											...
											if ( $courses_tag ) {
												...
												echo ' <i class="edu-tag"></i> ' . $courses_tag[0]->name . '</a>';
											}
										}

										if ( $course_rate ) {
											echo '<span class="star"><i class="edu-star-o"></i> ' . intval( $course_rate ) . '</span>';
										}
										?>
									<?php } ?>
								</div>
		...
		if ( $view_all_course && 'bottom' == $view_all_position ) {
			echo '<div class="wrapper-bottom-view-courses"><a class="view-all-courses position-bottom" href="' . esc_url( get_post_type_archive_link( 'lp_course' ) ) . '">' . esc_attr( $view_all_course ) . '</a></div>';
		}
		?>
	</div>
	<?php
