...
add_action(
	...
	function ( $post_id ) {
		...
		$available_seats = ! empty( $_POST['thim_course_available_seats'] ) ? absint( $_POST['thim_course_available_seats'] ) : '';
		$course_price    = ! empty( $_POST['thim_course_price'] ) ? sanitize_text_field( wp_unslash( $_POST['thim_course_price'] ) ) : '';
		$unit_price      = ! empty( $_POST['thim_course_unit_price'] ) ? sanitize_text_field( wp_unslash( $_POST['thim_course_unit_price'] ) ) : '';
		$class_size      = ! empty( $_POST['thim_course_class_size'] ) ? absint( $_POST['thim_course_class_size'] ) : '';
		$year_old        = ! empty( $_POST['thim_course_year_old'] ) ? sanitize_text_field( wp_unslash( $_POST['thim_course_year_old'] ) ) : '';
	...
	function thim_course_offline_manage_course_columns_content( $column ) {
		...
		switch ( $column ) {
			case 'thim_price':
				...
				echo wp_kses_post( $price . ' ' . $unit_price );
		}
	}
...
if ( ! function_exists( 'thim_course_offline_course_info' ) ) {
	function thim_course_offline_course_info() {
		...
		<div class="thim-course-info">
			...
			if ( ! empty( get_theme_mod( 'thim_learnpress_timetable_link' ) ) ) {
				echo '<div class="text-center"><a class="thim-timetable-link" target="_blank" href="' . esc_url( get_theme_mod( 'thim_learnpress_timetable_link' ) ) . '">' . esc_html( 'Courses Schedules', 'eduma' ) . '</a></div>';
			}
			?>
		</div>
		<?php
...
if ( ! function_exists( 'thim_course_offline_course_meta' ) ) {
	function thim_course_offline_course_meta() {
		...
		if ( $class_size || $year_old || $price ) {
			...
			if ( ! empty( $price ) ) {
				echo '<div class="course-price" itemprop="offers" itemscope itemtype="https://schema.org/Offer">';
				echo '<div class="value " itemprop="price" content="' . esc_attr( $price ) . '">' . esc_attr( $price ) . '</div>';
				echo ( ! empty( $unit_price ) ) ? '<div class="unit-price">' . esc_html( $unit_price ) . '</div>' : '';
				echo '</div>';
			}
...
function thim_course_offline_single_course_payment() {
	...
	if ( $price || $unit_price ) {
		?>
		<div class="course-price course-price-offline" itemprop="offers" itemscope itemtype="https://schema.org/Offer">
...
/**
 * Override Elementor LearnPress Widget + Thim Elementor Kit Price HTML
 */
function thim_render_custom_course_price_offline() {
	$price      = get_post_meta( get_the_ID(), 'thim_course_price', true );
	$unit_price = get_post_meta( get_the_ID(), 'thim_course_unit_price', true );
	...
	if ( in_array( $current_filter, [ 'learn-press/course/regular-price-html' ], true ) ) {
		return '';
	}

	ob_start();
	?>
	<span class="course-price course-price-offline">
		<span class="course-item-price">
			<span class="value price" itemprop="price" content="<?php echo esc_attr( $price ); ?>">
				<?php echo esc_html( $price ); ?>
			</span>
...
add_filter( 'learn-press/course/html-price', 'thim_render_custom_course_price_offline', 100 );
add_filter( 'learn_press_course_price_html', 'thim_render_custom_course_price_offline', 100 );
add_filter( 'learn_press_course_price_html_free', 'thim_render_custom_course_price_offline', 100 );
add_filter( 'learn-press/course/regular-price-html', 'thim_render_custom_course_price_offline', 100 );
...
function thim_kits_meta_data_course_offline( $string, $meta_data, $settings ) {
	...
	if ( in_array( 'thim_course_day_of_week', $meta_data ) ) {
		...
		if ( $day_of_week ) {
			$string .= '<span class="meta-feature">';
			if ( $settings['show_icon_meta_data'] == 'yes' ) {
				$string .= '<i class="edu-calendar-o"></i>';
			}
			if ( $settings['label_meta_data'] != 'yes' ) {
...
add_filter( 'learn-press/layout/list-courses/item/section-top', 'course_item_section_top_offline', 25, 3 );
function course_item_section_top_offline( $sections, $course, $settings ) {
	unset( $sections['read_more'] );

	return $sections;
}

add_filter( 'learn-press/layout/list-courses/item/section/bottom/end', 'course_item_section_bottom_offline', 25, 3 );
function course_item_section_bottom_offline( $sections, $course, $settings ) {
	ob_start();
	thim_course_offline_course_meta();
	$html_lesson             = ob_get_clean();
	$section['short_des']    = $sections['short_des'];
	$section['info_offline'] = $html_lesson;
