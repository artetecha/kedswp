	...
	function thim_paging_nav() {
		...
		if ( $links ) :
			...
			<div class="pagination loop-paginationasd">
				<?php echo $links; // phpcs:ignore WordPress.Security.EscapeOutput -- paginate_links() returns pre-escaped HTML ?>
			</div>
			<!-- .pagination -->
			<?php
		endif;
...
if ( ! function_exists( 'thim_entry_meta' ) ) :
	...
	function thim_entry_meta() {
		...
		$meta_tags = get_theme_mod( 'thim_blog_meta_tags', array( 'author', 'date', 'comment' ) );
		...
		<ul class="entry-meta">
			...
			foreach ( $meta_tags as $meta ) {
				switch ( $meta ) {
					...
					case 'date':
						// if (   is_single() || get_theme_mod( $display_layout, false ) ) {
							?>
							<li class="entry-date">
...
if ( ! function_exists( 'thim_archive_title' ) ) :
	...
	function thim_archive_title( $before = '', $after = '' ) {
		...
		if ( is_category() ) {
			...
			$title = sprintf( '%2$s', $tax->labels->singular_name, single_term_title( '', false ) );
		} elseif ( is_404() ) {
			$title = esc_html__( '404 Page', 'eduma' );
			if ( ! empty( get_theme_mod( 'thim_single_404_page_title', '' ) ) ) {
				$title = get_theme_mod( 'thim_single_404_page_title', '' );
			}
		} elseif ( is_search() ) {
...
if ( ! function_exists( 'thim_about_author' ) ) {
	function thim_about_author() {
			...
			<div class="author-wrapper lp-course-author">
				...
				<div class="author-bio">
					...
					<div class="author-description">
						<?php
						echo wpautop( wp_kses_post( get_the_author_meta( 'description', get_the_author_meta( 'ID' ) ) ) );
						?>
					</div>
				</div>
		...
		<?php
		if ( class_exists( 'LearnPress' ) && function_exists( 'thim_co_instructors' ) && get_post_type() == 'lp_course' ) {
